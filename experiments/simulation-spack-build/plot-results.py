...
def plot_histograms(scores, ideal_costs, outdir):
    ...
    for experiment, data in scores.items():
        cluster_costs = []
        if experiment in ["unmatched", "unmatched_at"]:
            continue
        for _, costlist in data["additional_runtime"]["additional_costs"].items():
    ...
    for experiment, data in scores.items():
        ...
        for package, costlist in data["additional_costs"]["additional_costs"].items():
            for i, c in enumerate(costlist):
                cc = data["additional_runtime"]["additional_costs"][package][i]
...
def parse_data(jobspecs, scores, unmatched):
    ...
    for experiment, data in scores.items():
        ...
        df.loc[idx, :] = [
            ...
            data["additional_runtime"]["total_additional_cost"],
